...
workers.validateCheckData = function(originalCheckData) {
	...
	if (originalCheckData.id &&
		...
		debug("Error: One of the checks "+originalCheckData.method+" is not properly formatted! Skipping it!");
	}
};
...
workers.alertUserToStatusChange = function(newCheckData) {
	...
	helpers.sendTwilioSms(newCheckData.userPhone, msg, function(err) {
		if (!err) {
			debug("Success: User was alerted of a state change in their check via SMS: " + msg);
		} else {
			debug("Error: Could not send an SMS alert to the user who had a state change in their check!");
		}
	});
};
...
workers.log = function(originalCheckData, checkOutcome, state, alertWarranted, timeOfCheck) {
	...
	_logs.append(logFileName, logString, function(err) {
		if (!err) {
			debug('Successfully logged into file!');
		} else {
			debug('Could not log into file!');
		}
	});
